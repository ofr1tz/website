---
title: History of OSM buildings in Kigali
author: Oliver Fritz
date: '2021-02-07'
draft: no
...
<!-- substitute with weblink-->
<iframe src = "https://maps.oliverfritz.de/kigali-osm-building-history/" width = "100%" height = "600"></iframe>

<a href="https://maps.oliverfritz.de/kigali-osm-building-history/" target="_blank">(Open Full Page Map)</a>

Thanks to [ohsome API](https://heigit.org/de/big-spatial-data-analytics/ohsome-3/){target="_blank"} and [mapview](https://r-spatial.github.io/mapview/){target="_blank"}, very little code is needed to make a web map visualising the evolution of building features in Kigali City on [OpenStreetMap](https://www.openstreetmap.org/relation/1708283#map=12/-1.9296/30.1297){target="_blank"} (OSM). The [httr](https://cran.r-project.org/web/packages/httr/vignettes/quickstart.html){target="_blank"} package provides convenient ways of sending POST requests.
...
Now a [data extraction](https://docs.ohsome.org/ohsome-api/v1/endpoints.html#extraction-endpoints){target="_blank"} query for yearly snapshots of building features within the boundaries of Kigali City can be posted to ohsome API.
...
And here is the processing chain from the response to the web map: The binary response is converted first to GeoJSON text, then to [simple features](https://r-spatial.github.io/sf/articles/sf1.html){target="_blank"} format. I decided to retain only building features that still existed in 2020, and selected the chronologically first version of each feature. The result is passed to mapview. 
...
map@map %>% 
    ...
    mapshot(url = here::here("../maps/kigali-osm-building-history/index.html"))
```
